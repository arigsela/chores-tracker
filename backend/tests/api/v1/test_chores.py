...
async def test_chore_cooldown_period(client: AsyncClient, db_session, parent_token, child_token, test_range_chore):
    ...
    assert response.status_code == 200
    assignment_id_2 = response.json()["assignment"]["id"]

    # Parent approves the chore again
    response = await client.post(
        f"/api/v1/assignments/{assignment_id_2}/approve",
        json={"reward_value": 3.00},
        headers={"Authorization": f"Bearer {parent_token}"}
    )
    assert response.status_code == 200
    ...
    # This would require manipulating the approval_date to be older than cooldown
    assignment = await db_session.get(ChoreAssignment, assignment_id_2)

    # Simulate passage of time - set approval date to be 8 days ago (beyond 7-day cooldown)
    old_date = datetime.now() - timedelta(days=8)
    assignment.approval_date = old_date
